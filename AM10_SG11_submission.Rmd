---
title: "AM10 SG11 Final Group Project Analyses"
...
```{r loading_data, message=FALSE,results='hide',warning=FALSE}
...
map_total <- euro_joined %>% 
  ...
  labs(title = '...but Czech Republic and France have Suffered Disproportionately in the Long-Term',subtitle = "Most Recent (24/11/2020) Total Cases per Million in Europe", x='',y=''
...
map_total_lockdown1 <- euro_joined1 %>% 
  ...
  labs(title = '...but Czech Republic and France have Suffered Disproportionately in the Long-Term',subtitle = "Cases per million ", x='',y=''
       # caption = 'Source:https://owid.data'
       ) +
  scale_fill_gradientn(name = NULL,colours = scale2,labels = c(0,'20K','40K','60K','80K')) +
  theme_fivethirtyeight() + 
...
map_total_lockdown2 <- euro_joined2 %>% 
  ...
  scale_fill_gradientn(name = NULL,colours = scale2,labels = c(0,'20K','40K','60K')) +
  theme_fivethirtyeight() + 
  theme(legend.position = 'right', legend.direction = 'vertical') +
  theme(plot.title = element_text(size = 13, face = "bold"), 
        plot.subtitle = element_text(size = 10), 
...
```{r stringency_graphs,fig.height = 3,fig.width = 6,message=FALSE,results='hide',warning=FALSE}
...
death_stringency <- covid_europe %>% 
  ...
  ggplot(aes(x=date))+
  facet_wrap(~factor(location, levels=c("Iceland","Finland","Sweden","Denmark","Germany","Switzerland","United Kingdom","France","Netherlands","Norway","Greece","Italy","Spain","Czech Republic","Poland","Luxembourg")))+
  geom_line(aes(y=new_deaths_smoothed_per_million),colour="orange")+
  geom_line(aes(y=stringency_index/coeff), colour="green")+
...
uk_1wave <- covid_europe %>% 
  ...
  geom_line(aes(y = x7_day_daily_test_change_per_million/10, colour = "Tests")) +
  ...
  # add annotate
  annotate(geom="text", x = as.Date('2020-04-17'),y = 200, label='First\nLockdown',color = 'black',size=3.5) +
  ...
  scale_y_continuous(limits = c(0,160),breaks = c(0,40,80,120,160),name = 'Daily Cases',labels = scales::comma,sec.axis = sec_axis(~.*10, name = "Daily Tests",labels = scales::comma))
...
poland_1wave <- covid_europe %>% 
  ...
  geom_line( aes(y = x7_day_daily_test_change_per_million/10, colour = "Daily Tests")) + #divide daily tests by 10 in order not to distort graph
  scale_color_manual(values=c("orange", "blue")) +
  #lockdown line
  ...
  annotate(geom="text", x = as.Date('2020-04-15'),y = 60,label='First\nLockdown',size=3.5) +
  
  theme_fivethirtyeight() +
  labs(title = 'Poland') +
  theme(plot.title = element_text(size = 13, face = "bold")) +
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "month") +
  scale_y_continuous(limits = c(0,160),breaks = c(0,40,80,120,160),labels = scales::comma ,sec.axis = sec_axis(~.*10,labels = scales::comma))  #multiply secondary axis by 10 in order to show actual results
...
italy_1wave <- covid_europe %>% 
  ...
  # add annotate
  annotate(geom="text", x = as.Date('2020-02-29'),y = 95, label='Lodi &\nPadua', color = 'black',size=3.5) +
  annotate(geom="text", x = as.Date('2020-03-21'),y = 95, label='North\nItaly', color = 'black',size=3.5)+
  annotate(geom="text", x = as.Date('2020-03-11'),y = 110, label='Lockdown', face= "bold", color = 'black',size=3.5)+
  ...
  scale_y_continuous(limits = c(0,160),breaks = c(0,40,80,120,160),labels = scales::comma,
                     sec.axis = sec_axis(~.*10,labels = scales::comma)) 
...
sweden_1wave <- covid_europe %>% 
  ...
  annotate(geom="text", x = as.Date('2020-04-16'),y = 100,label='Domestic\nTravel\nBan',size=3.5)+
  ...
  scale_y_continuous(limits = c(0,160),breaks = c(0,40,80,120,160),
                     labels = scales::comma,sec.axis = sec_axis(~.*10,labels = scales::comma)) 
...
france_1wave <- covid_europe %>% 
  ...
  annotate(geom="text", x = as.Date('2020-04-10'),y = 170,label='First\nLockdown',size=3.5)+
...
denmark_1wave <- covid_europe %>% 
  ...
  annotate(geom="text", x = as.Date('2020-04-10'),y = 550,label='First\nLockdown',size=3.5)+
  
  theme_fivethirtyeight() + 
  labs(title = 'Denmark') +
  theme(plot.title = element_text(size = 13, face = "bold")) +
  scale_x_date(labels = date_format("%m-%Y"), date_breaks = "month") +
  scale_y_continuous(limits = c(0,160),breaks = c(0,40,80,120,160),labels = scales::comma ,sec.axis = sec_axis(~.*10,labels = scales::comma)) 
```
...
title1 <- expression(atop(bold("When the First Wave Hits, the Sooner the Lockdown, the Better"), # title
                          scriptstyle("Daily Cases During the First Wave across UK, Poland, Italy, Sweden, France and Denmark"))) # subtitle

annotate_figure(wave1,
               top=text_grob(title1,size = 17),
               left = text_grob("Daily Cases Per Million",rot = 90,color = 'red',face = 'bold',size = 13),
               right = text_grob("Daily Tests Per Million",rot = 90,color = 'blue',face = 'bold',size = 13))
...
uk_2wave  <- covid_europe %>% 
  ...
  annotate(geom ="text", x = as.Date('2020-11-13'),y = 350, label='Second\nLockdown', color = 'black') +
  annotate(geom = 'text', x = as.Date('2020-10-23'), y = 350, label = 'New\nRestrictions', color = 'black') + 
...
poland_2wave<- covid_europe %>% 
  ...
  geom_vline(xintercept = as.Date('2020-11-24'), color = 'darkgrey', linetype = 'dashed')+ 
  annotate(geom="text", x = as.Date('2020-10-30'),y = 550,label='Second\nLockdown', size = 3.5) +
  annotate(geom="text", x = as.Date('2020-10-16'),y = 550,label='New\nRestrictions', size = 3.5) +
...
italy_2wave<- covid_europe %>% 
  ...
  geom_vline(xintercept = as.Date('2020-10-24'), color = 'darkgrey', linetype = 'dashed')+ 
  geom_vline(xintercept = as.Date('2020-11-24'), color = 'darkgrey', linetype = 'dashed')+ 
  annotate(geom="text", x = as.Date('2020-10-30'),y = 600,label='Second\nLockdown', size = 3.5) +
  annotate(geom="text", x = as.Date('2020-10-16'),y = 600,label='New\nRestrictions', size = 3.5) +
...
sweden_2wave <- covid_europe %>% 
  ...
  #new restrictions
  geom_vline(xintercept = as.Date('2020-10-09'), color = 'darkgrey', linetype = 'dashed')+ #add dashed line
  annotate(geom="text", x = as.Date('2020-10-20'),y = 400,label='\nNew\nRestrictions', size = 3.5) +
...
france_2wave <- covid_europe %>% 
  ...
  annotate(geom="text", x = as.Date('2020-11-10'),y = 500,label='Second\nLockdown',size=3.5)+
...
denmark_2wave <- covid_europe %>% 
  ...
  annotate(geom="text", x = as.Date('2020-11-7'),y = 1000,label='New\nRestrictions',size=3.5)+
