...
## Plot 3: Visualizing the trend of new cases after the first day new case number exceed 1000 in the top 5 countries with most cases in Europe
...
## Plot 4: Investigating the relationship between stringency measures and cases/deaths for prominent European countries
```{r}
...
## Plot 5.1: (UK) Visualizing the trend of cases and tests alongside key policy events 
...
UK_1wave /UK_2wave + plot_annotation(
  ...
  caption = 'Source:https://owid.data')
```
## Plot 5.2: (Poland) Visualizing the trend of cases and tests alongside key policy events 
...
## Plot 5.3: (Italy) Visualizing the trend of cases and tests alongside key policy events 
...
## Plot 5.4: (Sweden) Visualizing the trend of cases and tests alongside key policy events 
...
# PLOT 6 - RUNNING A STATISTICAL MODEL EVALUATING THE RELATIONSHIP BETWEEN STRINGENCY AND CASES/DEATHS IN ORDER TO PROPOSE THE OPTIMAL STRINGENCY MEASURE FOR A GIVEN COUNTRY AT A GIVEN TIME (THE PRESENT DAY / THE MOST RECENT DAY)
...
# total cases as y
lm1 <- lm(total_cases_per_million~aged_65_older+
            stringency_index:days_elapsed+stringency_index+
           stringency_index:location
           , data = covid_europe_lm)
...
plot_summs(lm1,inner_ci_level = .9,
           coefs = c('aged_65_older','stringency_index','stringency_index:days_elapsed',
                     'stringency_index:locationUnited Kingdom','stringency_index:locationGreece',
                     'stringency_index:locationSweden','stringency_index:locationItaly',
                     'stringency_index:locationPoland'))
...
# we can check variable importance as well
importance <- varImp(lm1, scale=TRUE)
plot(importance)
